...
class BgRemover:
    ...
    def __init__(
            ...
            preprocessor: transforms.transforms.Compose,
            device: str,
    ):
        self.model = model
    ...
    def mask_background(self, img: Image.Image) -> np.ndarray:
        ...
        # set background to all zeroes which is black color
        masked = np.where(bin_mask == 0, 0, np.array(img))
        return masked

    def remove_background(self, img: Image.Image, save_path: Path) -> None:
